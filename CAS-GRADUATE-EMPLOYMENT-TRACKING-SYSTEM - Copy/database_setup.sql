-- CAS Graduate Employment Tracking System Database Setup
...
-- Insert sample employment records (after profiles are created)
-- First, let's get the profile IDs and insert employment records
DO $$
DECLARE
    john_profile_id UUID;
    mike_profile_id UUID;
BEGIN
    -- Get John's profile ID
    SELECT id INTO john_profile_id FROM profiles WHERE email = 'john.doe@ssu.edu.ph' LIMIT 1;
    
    -- Get Mike's profile ID
    SELECT id INTO mike_profile_id FROM profiles WHERE email = 'mike.johnson@ssu.edu.ph' LIMIT 1;
    
    -- Insert John's employment record
    IF john_profile_id IS NOT NULL THEN
        ...
        VALUES (john_profile_id, 'Software Developer', 'Tech Solutions Inc.', 'Information Technology', 'Full-time', '25,000 - 35,000', '2023-06-01', true, 'Highly Relevant', 'Manila')
        ON CONFLICT DO NOTHING;
    END IF;
    
    -- Insert Mike's employment record
    IF mike_profile_id IS NOT NULL THEN
        INSERT INTO employment_records (profile_id, job_title, company_name, industry, employment_type, salary_range, start_date, is_current, job_relevance, location)
        VALUES (mike_profile_id, 'Data Analyst', 'Analytics Corp', 'Data Science', 'Full-time', '30,000 - 40,000', '2023-07-01', true, 'Highly Relevant', 'Cebu')
        ON CONFLICT DO NOTHING;
    END IF;
END $$;